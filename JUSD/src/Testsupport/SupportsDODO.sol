...
contract SupportsDODO {
    ...
    function swap(uint256 amount, address token) external {
        ...
        uint256 value = amount * IPriceChainLink(tokenPrice[token]).getAssetPrice() / 1e18;
        IERC20(USDC).transfer(msg.sender, value);
    }

    function getSwapData(uint256 amount, address token) external pure returns (bytes memory) {
